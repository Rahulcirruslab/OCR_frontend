  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.job_id) {
        toast({
          title: "Error",
          description: "Job ID not received from backend. Please check your API.",
          variant: "destructive",
        });
        setLoading(false);
        return;
      ...
      setTimeout(() => {
        ...
        navigate(`/results/${response.job_id}${refId ? `?ref_id=${refId}` : ''}`);      
      }, 1500);
    } catch (error: any) {
      const errorMessage = error.response?.data?.message || error.response?.data?.detail || "Failed to process file. Please try again.";
      toast({
        title: "Error",
  ...
  return (
    <div className="min-h-screen pt-24 pb-12">
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        >
          <h1 className="text-4xl font-bold mb-2 gradient-text">Upload Test Paper</h1>
          ...
          >
            ...
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              ...
              >
                {loading ? "Processing with AI..." : "Process with Agentic AI"}
              </Button>
            </motion.div>
          </form>
        </motion.div>
