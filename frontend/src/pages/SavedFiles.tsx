...
import { FileText, RefreshCw, Calendar, User, Trash2, X, AlertTriangle } from 'lucide-react';
...
const SavedFiles: React.FC = () => {
  ...
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; jobId: string; examName: string } | null>(null);
  ...
  const handleReprocess = async (jobId: string) => {
    try {
      ...
      setToast({ show: true, message: 'Failed to reprocess file', type: 'error' });
    } finally {
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <AnimatePresence>
          {toast?.show && (
            ...
            >
              <div className={`glass-card rounded-lg p-4 shadow-lg border-l-4 ${
                ...
                <button onClick={() => setToast({ ...toast, show: false })} className="ml-2">
            ...
            <>
              ...
              >
                <div className="glass-card rounded-2xl p-6 max-w-md w-full shadow-2xl border border-border/50">
                  ...
                  {/* Content */}
                  <h3 className="text-xl font-bold text-center mb-2">Delete Test Paper?</h3>
                  <p className="text-muted-foreground text-center mb-6">
                    Are you sure you want to delete <span className="font-semibold text-foreground">"{deleteModal.examName}"</span>? This action cannot be undone.
                  </p>
              ...
              {files.map((file, index) => (
                ...
                >
                  ...
                  <div className="flex items-start mb-4">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium whitespace-nowrap ${getStateColor(file.state)}`}>
                      {file.state}
                    </span>
                  </div>
                  ...
                  <div className="space-y-2 mb-4 text-sm">
                    ...
                    <div className="text-xs text-muted-foreground truncate" title={file.job_id}>
                      <strong className="text-foreground">ID:</strong> {file.job_id}
                    </div>
                  ...
                  <div className="flex space-x-2">
                    ...
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => handleDeleteClick(file.job_id, file.exam_name)}
