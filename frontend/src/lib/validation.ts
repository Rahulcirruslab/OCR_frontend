...
export const studentUploadSchema = z.object({
  student_name: z.string().trim().min(1, "Student name is required").max(100, "Name too long"),
  student_id: z.string().trim().min(1, "Student ID is required").max(50, "ID too long"),
  ...
  total_marks: z.string().refine((val) => !isNaN(Number(val)) && Number(val) > 0, {
    message: "Total marks must be a positive number",
  }),
});

export const referenceUploadSchema = z.object({
  teacher_name: z.string().trim().min(1, "Teacher name is required").max(100, "Name too long"),
  teacher_id: z.string().trim().min(1, "Teacher ID is required").max(50, "ID too long"),
  exam_name: z.string().trim().min(1, "Exam name is required").max(100, "Exam name too long"),
  subject: z.string().trim().min(1, "Subject is required").max(100, "Subject name too long"),
  total_marks: z.string().refine((val) => !isNaN(Number(val)) && Number(val) > 0, {
    message: "Total marks must be a positive number",
